    ...
    namespace stat {
        ...
        public:
            ...
            void _write_progress(const std::string& prefix, const EA& ea) const
            {
				...
				// z gives the colour of the point in x, y plane, is passed to python as kwarg c in matplotlibcpp.h
			...
			std::vector<std::vector<double> > create_color( const std::vector<double>& x) const
			{
				...
				{
					color.push_back(
						// curly braces to initialise an object of std::vector type and push it back
						{
							(sin((x[i]-min)/diff*2*M_PI + 1.3*M_PI) / 2.0+0.5) * 0.8 + 0.1,
